...
//問題文、答えが入力されていないときのエラーメッセージ
function input_check() {
	if($('.qTxt').val() === ''){
		document.getElementById('error').textContent = '問題文を入力してください';
		$('.qTxt').toggleClass('idError');
		return false;
	}else if($('.addTxt').val() === ''){
		document.getElementById('error').textContent = '答えを入力してください';
		$('.addTxt').toggleClass('idError');
		return false;
	}else{
		return true;
	}
